...
@Component({
  selector: 'app-treatment-history-hospital',
  templateUrl: './treatment-history-hospital.component.html',
  styleUrls: ['./treatment-history-hospital.component.scss']
})
export class TreatmentHistoryHospitalComponent implements OnInit {
  urn: any;
  trtData: any = [];
  token: any;
  ...
  getDetails(transactionId, claimId) {
    ...
    if (clmId != null || clmId != undefined) {
      ...
      this.route.navigate([]).then(result => { window.open(environment.routingUrl+'/trackingdetailshospital'); });
    } else {
