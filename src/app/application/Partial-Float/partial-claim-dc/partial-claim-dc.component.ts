...
@Component({
  selector: 'app-partial-claim-dc',
  templateUrl: './partial-claim-dc.component.html',
  styleUrls: ['./partial-claim-dc.component.scss']
})
export class PartialClaimDcComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle("DC Partial Claim Approval");
  ...
  Search(){
    ...
    this.partialclaimserv.getpartialclaimlistdc(this.formdate, this.todate, this.user.userId,this.schemeidvalue,this.schemecategoryidvalue).subscribe((data: any) => {
      if(data.status==200){
