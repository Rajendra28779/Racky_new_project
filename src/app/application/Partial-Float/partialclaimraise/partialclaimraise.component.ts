...
@Component({
  selector: 'app-partialclaimraise',
  templateUrl: './partialclaimraise.component.html',
  styleUrls: ['./partialclaimraise.component.scss']
})
export class PartialclaimraiseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle("Partial Claim To Raise");
  ...
  getClaimDetails(){
    ...
    this.partialclaimserv.getlistofpartialclaim(this.formdate, this.todate, this.user.userName,this.schemeidvalue,this.schemecategoryidvalue).subscribe((data: any) => {
      if(data.status==200){
  ...
  heading = [
    ['Sl#', 'URN', 'Patient Name', 'Case Number', 'Claim Number', 'Package Code','Package Name',
     'Actual Date Of Admission', 'Actual Date Of Discharge', 'Total Amount Claimed',
     'CPD Approved Amount', 'SNA Approved Amount', 'Difference Amount']
  ];
  report:any=[];
  sno:any=[];
  ...
  downloadList(no: any) {
    ...
    for (let i = 0; i < this.list.length; i++) {
      ...
      this.sno.push(this.convertCurrency(v.totalAmountClaim)); // Total Amount Claimed(₹)
      this.sno.push(this.convertCurrency(v.cpdApprovedAmount)); // CPD Approved Amount(₹)
      this.sno.push(this.convertCurrency(v.snaApprovedAmount)); // SNA Approved Amount(₹)
    ...
    if (no == 1) {
      ...
      doc.setFontSize(20);
      doc.text("Partial Claim list", 120, 15);
      doc.setFontSize(13);
      ...
      doc.save('Partial Claim list.pdf');
    }
  }
