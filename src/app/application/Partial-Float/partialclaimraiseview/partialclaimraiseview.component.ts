...
@Component({
  selector: 'app-partialclaimraiseview',
  templateUrl: './partialclaimraiseview.component.html',
  styleUrls: ['./partialclaimraiseview.component.scss']
})
export class PartialclaimraiseviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle("Partial Claim Tracking");
    ...
    $('input[name="toDate"]').attr('placeholder', 'To Date *');
      ...
      this.formdate = localStorage.getItem("fromdate1");
      this.todate = localStorage.getItem("todate1");
      if(this.formdate){
  ...
  getClaimDetails(){
    ...
    this.partialclaimserv.getlistofpartialclaimview(this.formdate, this.todate, this.user.userName,this.schemeidvalue,this.schemecategoryidvalue).subscribe((data: any) => {
      if(data.status==200){
  ...
  action(item:any){
    localStorage.setItem("fromdate1", this.formdate);
    localStorage.setItem("todate1", this.todate);
  ...
  downloadList(no: any) {
    ...
    for (let i = 0; i < this.list.length; i++) {
      ...
      this.sno.push(this.convertCurrency(v.partialclaimAmount)); // Difference Amount(₹)
