...
@Component({
  selector: 'app-partial-claim-dc-noncompliance',
  templateUrl: './partial-claim-dc-noncompliance.component.html',
  styleUrls: ['./partial-claim-dc-noncompliance.component.scss']
})
export class PartialClaimDcNoncomplianceComponent implements OnInit {
  ...
  Search(){
    ...
    this.partialclaimserv.getpartialclaimdcnoncomp(this.formdate, this.todate, this.user.userId,this.schemeidvalue,this.schemecategoryidvalue).subscribe((data: any) => {
      if(data.status==200){
...
heading = [
  ['Sl#', 'URN','Patient Name', 'Case No','Claim No', 'Hospital Name', 'Package Code','Package Name','Actual Date Of Admission',
   'Actual Date Of Discharge', 'Difference Amount ', 'Partial Claim Amount ','SNA Action On']
];
