...
@Component({
  selector: 'app-partial-claim-dc-view',
  templateUrl: './partial-claim-dc-view.component.html',
  styleUrls: ['./partial-claim-dc-view.component.scss']
})
export class PartialClaimDcViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle("Partial Claim Approval");
  ...
  Search(){
    ...
    this.partialclaimserv.getpartialclaimlistdcview(this.formdate, this.todate, this.user.userId,this.schemeidvalue,this.schemecategoryidvalue).subscribe((data: any) => {
      if(data.status==200){
...
heading = [
  ['Sl#', 'URN','Patient Name', 'Case No','Claim No', 'Hospital Name', 'Package Code','Package Name','Actual Date Of Admission',
   'Actual Date Of Discharge', 'Difference Amount ', 'Partial Claim Amount ']
];
...
downloadList(no: any) {
  ...
  if (no == 1) {
    ...
    doc.setFontSize(20);
    doc.text("DC Partial Claim View", 120, 15);
    doc.setFontSize(13);
    ...
    doc.save('DC_Partial_Claim_View.pdf');
  }
}
...
scheme: any;
  ...
  action(item:any){
    ...
    this.route.navigate(['application/dcPartialClaim/action'], navigationExtras);
  }
}
