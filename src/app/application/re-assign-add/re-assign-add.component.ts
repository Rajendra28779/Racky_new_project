...
@Component({
  selector: 'app-re-assign-add',
  templateUrl: './re-assign-add.component.html',
  styleUrls: ['./re-assign-add.component.scss']
})
export class ReAssignAddComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Re-assign CCE');
  ...
  getMobileNoActiveStatusNotConnected() {
    this.cceService.getAllReassignData(this.user.userId).subscribe((data: any) => {
  ...
  SubmitCreate() {
    ...
    for (var j = 0; j < l.length; j++) {
      ...
      if (this.selected != null && this.selected[j].actStat == 1) {
        ...
        if (this.notConnectedForm.value.que1 == null || this.notConnectedForm.value.que1 == "" || this.notConnectedForm.value.que1 == undefined) {
          $("#que1").focus();
          this.swal("Info", "Please Select Are you receiving cashless treatment under ?", 'info');
          return;
        }
    ...
    if (this.selected == null || this.selected.length == 0 || this.selected == undefined || this.selected.length > 1) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.cceService.saveReassign(this.notConnectedForm.value).subscribe((data: any) => {
          this.notConnectedForm = data;
          if (data.status == "Success") {
            this.swal("Success", data.message, "success");
            // this.router.navigate(['/application/reAssignview']);
            this.ResetForm();
          }
