...
@Component({
  selector: 'app-dgo-query-bucket',
  templateUrl: './dgo-query-bucket.component.html',
  styleUrls: ['./dgo-query-bucket.component.scss']
})
export class DgoQueryBucketComponent implements OnInit {
  user: any;
  pageElement: number;
  currentPage: number;
  months: string;
  ...
  downloadReport() {
    ...
    for (var i = 0; i < this.cceOutboundData.length; i++) {
      ...
      this.callCenter.dcRemarks = item.dgoRemarks ? item.dgoRemarks : '-NA-'
