...
@Component({
  selector: 'app-pending-grievance-application',
  templateUrl: './pending-grievance-application.component.html',
  styleUrls: ['./pending-grievance-application.component.scss']
})
export class PendingGrievanceApplicationComponent implements OnInit {
  user:any;
  ...
  jsonurlDC  = 'assets/js/_configs/pendingApplicationsForDCGrievance.config.json';
  tablist: any;
  tabDataId: any;
  utillist: any;
  messaageslist: any;
  ...
  param: { itemId: any; pendingAt: number; pageType: string; profileId: number; serviceId: number;
    mode :any,formDate:any,toDate:any,stateCode:any,distCode:any,hostCode:any,grievancebyId:any,lstAct: any,pendingApplication:any,DCcaseType:any,grievancemediumId:any,
    dcUserIdList:any,hospitalCodeList:any,GrvcaseType:any,selectedItem:any,selectedItem1:any
  };
  record : any;
  packageData: any;
  ...
  ngOnInit() {
   ...
   if( this.url.includes('192.168.10.46')){
    ...
    if( this.url.includes('bskycms.odisha.gov.in')){  //pre prod
      ...
      this.tempurl='https://bskygrvpp.odisha.gov.in:8443/#/website/formPreview/';
    }
    this.headerService.setTitle('View Pending Applications');
    if(localStorage.getItem('routepage') != 'N'){
    ...
    if (
      ...
      this.dataRequest == ''
            ...
            if(this.userRole == 99){ //GO

              this.loadconfigOfficer(583);
            }
            if(this.userRole == 32){ //DGO
              this.loadconfigSHAS();
            }
            if(this.userRole == 37 || this.userRole == 41){ //GrievanceCEO

              this.loadconfigCEO(583);
            }
            if(this.userRole == 38){ //SHASJointDirector

              this.loadconfigCEO(583);
            }
            if(this.userRole == 12){ //CDMO

              this.loadconfigCDMO(583);
            }
            if(this.userRole == 6){ //DC
              this.loadconfigdc();
            }
            this.currentPage = 1;
            this.pageElement = 10;
            ...
            if(this.userRole == 37){ //GrievanceCEO
            ...
            this.selectedItems=this.dataRequest.selectedItem;
            this.selectedItems1=this.dataRequest.selectedItem1;
  ...
  OnChangeDistrict(event) {
    ...
    this.snoCreateService.getDCDetailsByStateAndDist(this.stateCode,this.distCode).subscribe(
      (response) => {
        ...
        if(this.dataRequest != null && this.dataRequest!="" && this.dataRequest!=undefined){
          if (
            this.dataRequest.selectedItem?.length != 0 && this.dataRequest.stateCode==this.stateCode && this.dataRequest.distCode==this.distCode
          ) {
            this.selectedItems = this.dataRequest.selectedItem;
  ...
  onChangeDClList(dcList:any[]){
    ...
    this.appListService.getHospitalDetailsFromDCUserId(dcList).subscribe((response: any) => {
      ...
      if (response.status == 'success') {
        ...
        if (this.dataRequest != null && this.dataRequest != "" && this.dataRequest != undefined) {
          if (
            this.dataRequest.selectedItem1?.length != 0 && this.dataRequest.stateCode == this.stateCode && this.dataRequest.distCode == this.distCode && this.checkDcList(this.dataRequest.dcUserIdList, this.DCUserIdList)
          ) {
            this.selectedItems1 = this.dataRequest.selectedItem1;
            this.onSelectAll1(this.selectedItems1);
          }
        }
      }
    })
  }
  checkDcList(dcList1:any[],dcList2:any[]){
    const set2 = new Set(dcList2);
    return dcList1.length === dcList2.length && dcList1.every(value => set2.has(value));
    }
  DCUserIdList: any = [];
  DCNameList: any = [];
  dcUserObj: any;
  ...
  loadconfigOfficer(formId: any) {
    let encSchemeStr = this.encDec.encText(formId.toString());
    this.route.navigate([
      '/application/grievance-application-summary',
      encSchemeStr,
    ]);
  ...
  loadconfigCEO(formId:any) {
    let encSchemeStrs = this.encDec.encText(formId.toString());
    this.route.navigate([
      '/application/grievance-summary-ceo',
      encSchemeStrs,
    ]);
...
pageItemChange() {
  ...
  searchData(){
    this.findDcList(this.selectedItems);
    this.onSelectAll1(this.selectedItems1);
    ...
    mode : 'search',formDate:fromdate,toDate:todate,stateCode:stateCode,distCode: distCode,hostCode:hostCode,grievancebyId:grievancebyId,lstAct: lstAct,pendingApplication:pendingAt,DCcaseType:DCcaseType,grievancemediumId:grievancemediumId,
    dcUserIdList:this.DCUserIdList,hospitalCodeList:this.hospitalCodeList,GrvcaseType:GrvcaseType,selectedItem:this.selectedItems,selectedItem1:this.selectedItems1
  };
    ...
    this.appListService.getApplicationList(this.param).subscribe((res) => {
      ...
      for (let i in this.dataResult) {
         if (this.dataResult[i].INTSENTFROM == 13) {
  ...
  notingDetails(formParms: any) {
    ...
    localStorage.setItem('routepage', "N");
  ...
  downloadReport() {
    ...
    TableUtil.exportListToExcelWithFilter(this.report, 'Grievance_application_DGO', this.heading, filter);
  }
  ...
  downloadPdf() {
    ...
    doc.text("Generated By: "+this.user.fullName, 180, 40);
    doc.text("Grievance_application_DGO", 110, 50);
    ...
    doc.save('Grievance_application_DGO.pdf');
}
convertDate(date) {
...
downloadReport1() {
  ...
  for (let i = 0; i < this.dataResult.length; i++) {
    ...
    this.SNALISt1.GrievanceNo=sna.VCHAPPLICATIONNO;
    this.SNALISt1.contactNo=sna.CONTACT_NO;
    this.SNALISt1.state = sna.STATECODE;
    this.SNALISt1.district=sna.DISTRICTCODE;
    this.SNALISt1.hospital = sna.HOSPITALDETAILS;
    this.SNALISt1.registerBy = sna.INTUPDATEDBY;
    this.SNALISt1.AppliedDate = sna.STMCREATEDON;
    this.SNALISt1.lastActionBy= sna.INTSENTFROM;
    this.SNALISt1.status=status;
    this.report.push(this.SNALISt1);
  }
  console.log(this.param);
  let stateName = 'All',grievance_By='All';
  let grievance_type="All";
  let fromDate = $('#formdate').val().toString();
  let toDate = $('#todate').val().toString();
  if(this.param!=undefined){
    fromDate=this.param.formDate;
  ...
  grievance_By=this.getGrievanceBy(this.param.grievancebyId);
  grievance_type=this.getGrievanceType(this.param.DCcaseType);
}
    let filter =[];
    filter.push([['From Date', fromDate]]);
    filter.push([['To Date', toDate]]);
    ...
    filter.push([['Grievance By', grievance_By]]);
    if(this.param.grievancebyId==9){
      filter.push([['Grievance From', grievance_type]]);
    }

  TableUtil.exportListToExcelWithFilter(this.report, 'Grievance_application_DC', this.heading1, filter);
}
downloadPdf1() {
  let doc = new jsPDF('l', 'mm', [240, 170]);
  doc.setFontSize(12);
  let stateName = 'All',grievance_By='All',grievance_type="All";
  let fromDate = $('#formdate').val().toString();
  let toDate = $('#todate').val().toString();
  if(this.param!=undefined){
    fromDate=this.param.formDate;
  ...
  doc.text('From Date:'+fromDate, 10, 10);
  doc.text('To Date:'+toDate, 180, 10);
  doc.text('State Name:'+stateName, 10, 20);
  doc.text('Grievance By:'+grievance_By, 10, 30);
  if(this.param.grievancebyId==9){
  doc.text('Grievance From:'+grievance_type, 180, 30);
  }
  doc.text("Generated On: "+ this.convertDate(new Date()), 10, 40);
  doc.text("Generated By: "+ this.user.fullName, 180, 40);
  doc.text("Grievance_application_DC", 110, 50);
  let col = this.heading1;
  let rows = [];
  let claim: any;
  for(let i=0;i<this.dataResult.length;i++) {
    claim = this.dataResult[i];
  ...
  doc.save('Grievance_application_DC.pdf');
}
loadconfigCDMO(formId:any) {
  let encSchemeStrs = this.encDec.encText(formId.toString());
  this.route.navigate([
    '/application/grievance-application-cdmo',
    encSchemeStrs,
  ]);
