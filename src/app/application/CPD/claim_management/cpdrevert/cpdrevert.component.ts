...
@Component({
  selector: 'app-cpdrevert',
  templateUrl: './cpdrevert.component.html',
  styleUrls: ['./cpdrevert.component.scss']
})
export class CpdrevertComponent implements OnInit {
  ...
  revertClaimCount: any;
  authMode: any;
  allRecordsSize: any;
  schemeidvalue: any = 1
  schemeName: any
  constructor(private cpdService: CreatecpdserviceService, public headerService: HeaderService, public router: Router,
    private service: DynamicreportService, private sessionService: SessionStorageService, public packageDetailsMasterService: PackageDetailsMasterService, private encryptionService: EncryptionService) { }
  ...
  ngOnInit(): void {
    ...
    if (searchFilterParameters != null && searchFilterParameters.type == 'REVERT') {
      ...
      this.getCPDCliamRevertList();
      this.getCPDClaimRevertListCountData();
    }
  }
  ...
  getCPDClaimRevertListCount(userId, orderValue, fromDate, toDate, authMode, trigger: any, schemeid: any, schemecategoryid: any) {
    this.cpdService.getCPDClaimRevertListCount(userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid).subscribe(data => {
      if (data != null && data.status == 'success') {
        this.revertClaimCount = data.data.revertClaimCount;
      } else {
        this.revertClaimCount = 0;
        this.swal('Error', 'Failed to Fetch Revert Claim Count', 'eror');
      }
    });
  }

  getCPDCliamRevertList() {
    ...
    this.cpdService.getCPDCliamRevertListSearchFilter(this.user.userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid).subscribe(data => {
      this.claimlist = data;
  ...
  getCPDClaimRevertListCountData() {
    let orderValue = $('#orderValue').val();
    let fromDate = $('#fromDate').val();
    let toDate = $('#toDate').val();
    let authMode = $('#authMode').val();
    ...
    this.getCPDClaimRevertListCount(this.user.userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid);
  }
  ...
  onAction(claimID: any, transactionID: any, URN: any, transClaimId: any, authorizedcode: any, hospitalcode: any, actualDate: any, caseNo: any, claimNo: any) {
    ...
    this.router.navigateByUrl('/application/cpdrevert', { skipLocationChange: true }).then(() =>
      this.router.navigate(['/application/cpdrevert/action']));
  ...
  searchFilter() {
    ...
    this.sessionService.encryptSessionData("searchFilterParameters", searchFilterParameters);
  }
  ...
  downloadReport(type: any) {
    let SlNo = 1;
    let report = [];
    let heading = [['Sl#', 'Claim No', 'URN', 'Case No', 'Invoice No', 'Patient Name', 'Actual Date of Admission', 'Actual Date of Discharge', 'SNA Reverted Date', 'Action to be Taken By']];
    let months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    if (type == 'excel') {
      let claim: any;
      this.claimlist.forEach(element => {
        claim = {
          ...
          "SNA Reverted Date": this.formatDate(element.revisedDate),
          "Action to be Taken By": this.formatDate(element.takenDate)
        }
        report.push(claim);
        SlNo++;
      });
      TableUtil.exportListToExcel(report, "SNA Revert List", heading);
    } else if (type == 'pdf') {
      if (this.claimlist.length > 0) {
        ...
        doc.text('SNA Reverted List', 220, 30);
        doc.setFontSize(10);
        ...
        doc.save('SNA Reverted List.pdf');
      } else {
