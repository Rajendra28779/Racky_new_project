...
@Component({
  selector: 'app-cpd-case-resettlement-action',
  templateUrl: './cpd-case-resettlement-action.component.html',
  styleUrls: ['./cpd-case-resettlement-action.component.scss'],
})
export class CpdCaseResettlementActionComponent implements OnInit {
  user: any;
  data: any;
  caseId: any;
  caseNo: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Case for CPD Re-Settlement');
  ...
  getCPDCaseDetails() {
    ...
    this.allocateService.getCPDResettlementDetails(requestData).subscribe(
      (response) => {
        ...
        if (this.responseData.status == 'success') {
          ...
          this.caseActionList = resData.caseActionList;
          console.log(resData);
          this.packageDetails.forEach((item) => {
            item.originalAmount = item.hosClaimAmount; // Store original value
            item.modifiedAmount = item.hosClaimAmount; // Initialize modified amount
          });
          // this.packageDetails.forEach(element => {
          //   this.approvedAmount = Number(element.hosClaimAmount)+this.approvedAmount;
          // });
        } else {
  ...
  showProcedureName(text, index) {
    $('#procedurename' + index).text(text);
    $('#showMoreprocedurename' + index).empty();
    $('#showMoreprocedurename1' + index).append(
      '<div style="cursor: pointer; color: #1d89c9">Show Less</div>'
    );
  }
  hideProcedureName(text, index) {
    if (text.length > 15) {
      $('#procedurename' + index).text(text.substring(0, 15) + '...');
      $('#showMoreprocedurename1' + index).empty();
      $('#showMoreprocedurename' + index).empty();
      $('#showMoreprocedurename' + index).append(
        '<span style="cursor: pointer; color: #1d89c9">Show More</span>'
      );
    }
  }
  showDesc(text, index) {
    $('#desc' + index).text(text);
    $('#showdesc' + index).empty();
    $('#showdesc1' + index).append(
      '<div style="cursor: pointer; color: #1d89c9">Show Less</div>'
    );
  }
  hideDesc(text, index) {
    if (text.length > 15) {
      $('#desc' + index).text(text.substring(0, 15) + '...');
      $('#showdesc1' + index).empty();
      $('#showdesc' + index).empty();
      $('#showdesc' + index).append(
        '<span style="cursor: pointer; color: #1d89c9">Show More</span>'
      );
    }
  }
  ...
  submitDetails(event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.allocateService.cpdCaseResettlementAction(data).subscribe(
          (data: any) => {
            console.log(data);
            ...
            this.route.navigate(['/application/cpdcaseresettlement']);
          },
          (error) => {
            console.log(error);
  ...
  getTabDataDetails(action) {
    ...
    this.allocateService.getTabDataDetails(requestData).subscribe(
      (response) => {
        this.responseData = response;
        if (this.responseData.status == 'success') {
          let resData = this.responseData.data;
  ...
  onAction(data) {
    ...
    this.route.navigate(['/application/cpdfreshcaseclaimdetails']);
  }
