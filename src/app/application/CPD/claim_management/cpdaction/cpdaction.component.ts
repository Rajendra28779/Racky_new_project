...
@Component({
  selector: 'app-cpdaction',
  templateUrl: './cpdaction.component.html',
  styleUrls: ['./cpdaction.component.scss'],
})
export class CpdactionComponent implements OnInit {
  [x: string]: any;
  childmessage: any;
  id: any = null;
  claimId: string;
  ...
  ngOnInit(): void {
    $("#modal").hide();
    this.headerService.setTitle('CPD Approval');
    ...
    this.getIndividualClaimDetails(
      this.URN,
      this.transactionid,
      this.claimId,
      this.authorizedCode,
    ...
    this.cpdService
      ...
      .subscribe((data) => {
        ...
        for (let i = 0; i < this.claimlist1.result.length; i++) {
          ...
          this.claimDetailsForm.controls['invoiceNumber'].setValue(claim1.invoiceNo);
          this.claimDetailsForm.controls['patientName'].setValue(claim1.patientName);
          this.claimDetailsForm.controls['age'].setValue(claim1.age);
          this.claimDetailsForm.controls['gender'].setValue(claim1.gender);
          this.claimDetailsForm.controls['Address'].setValue(claim1.patientAddress);
          this.claimDetailsForm.controls['noofdays'].setValue(claim1.noOfDays);
          this.claimDetailsForm.controls['admissiondate'].setValue(claim1.dateOfAdmission);
          this.claimDetailsForm.controls['packageCost'].setValue(this.currencyPipe.transform(claim1.packageCost, 'INR'));
          this.claimDetailsForm.controls['blockedAmount'].setValue(this.currencyPipe.transform(claim1.hospitalClaimedAmount, 'INR'));
          this.claimDetailsForm.controls['procedureName'].setValue(claim1.procedureName);
          this.claimDetailsForm.controls['packageName'].setValue(claim1.packageName);
          this.claimDetailsForm.controls['totalClaimAmount'].setValue(claim1.hospitalClaimedAmount);
  ...
  downloadAction(
    ...
    } else if (docStatus == 'mortaudit') {
      this.mortaudit = 'true';
      this.docCount++;
      this.morauditcheck = true;
      this.moraudituncheck = false;
    }
  ...
  submitClaimnew(actionType: String) {
    ...
    if (this.selectedRemarkId == 0 || this.selectedRemarkId == null || this.selectedRemarkId == '' || this.selectedRemarkId == undefined) {
      Swal.fire('', 'Please select Remark', 'error');
      $('#reasonId').focus();
      return;
    } else if (
      ...
      this.resuestedData = {
        ...
        mortality: mortalId,
        timingLogId: this.timingLogId,
        dischargeSlip: this.actionDtls.dischargeSlip != null ? this.actionDtls.dischargeSlip : '',
        additinalSlip: this.actionDtls.additinalSlip != null ? this.actionDtls.additinalSlip : '',
        preSurgerySlip: this.actionDtls.preSurgerySlip != null ? this.actionDtls.preSurgerySlip : '',
        postSurgerySlip: this.actionDtls.postSurgerySlip != null ? this.actionDtls.postSurgerySlip : '',
        intraSurgery: this.actionDtls.intraSurgery != null ? this.actionDtls.intraSurgery : '',
        specimenPhoto: this.actionDtls.specimenPhoto != null ? this.actionDtls.specimenPhoto : '',
        patientPhoto: this.actionDtls.patientPhoto != null ? this.actionDtls.patientPhoto : '',
        admissionSlip: this.actionDtls.admissionSlip != null ? this.actionDtls.admissionSlip : '',
  ...
  getCPDTrigger() {
    ...
    console.log("all detaiks" + this.actionDtls);
  ...
  onAction(claimID: any, transactionID: any, URN: any, transClaimId: any, authorizedcode: any, hospitalcode: any, actualDate: any, caseNo: any, claimNo: any) {
    ...
    window.open(environment.routingUrl + '/application/cpdapproval/action');
  ...
  onHospitalPackagePatternClick() {
    let transactionid = this.transactionid;
    this.sessionService.encryptSessionData('transactionid', transactionid);
    this.router.navigate([]).then((result) => {
      window.open(environment.routingUrl + '/swasthyamitrareviewpatientdetails');
    });
  }
  isActionAuthAllow(){
  ...
  actionFlag:any;
  submitClaimAction(){
    this.cpdService.cpdClaimActionRequestnew(this.resuestedData).subscribe(
      (response: any) => {
        var result = response;
        ...
        this.router.navigate(['/application/cpdapproval']);
      },
      (error) => {
        Swal.fire(
          'Some error occured', 'Could not ' + this.actionFlag + ' ', 'error'
        );
      }
    );
  }
