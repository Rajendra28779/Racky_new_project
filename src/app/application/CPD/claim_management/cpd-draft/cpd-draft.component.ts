...
import { DynamicreportService } from 'src/app/application/Services/dynamicreport.service';
...
declare let $: any;
@Component({
  selector: 'app-cpd-draft',
  templateUrl: './cpd-draft.component.html',
  styleUrls: ['./cpd-draft.component.scss']
})
export class CpdDraftComponent implements OnInit {
  claimlist: any = [];
  userId: string;
  trigger: any
  triggerList: any[] = [];
  ...
  ngOnInit(): void {
    this.getTriggerList();
    this.headerService.setTitle('Claim Draft Application');
    ...
    if (searchFilterParameters != null && searchFilterParameters.type == 'APPROVAL') {
  ...
  getCPDApprovalListCount(userId, orderValue, fromDate, toDate, authMode, trigger: any, schemeid: any, schemecategoryid: any) {
    this.cpdService.getCPDApprovalListCount(userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid).subscribe(data => {
      if (data != null && data.status == 'success') {
        this.freshClaimCount = data.data.freshClaimCount;
      } else {
        this.swal('Error', 'Failed to Fetch Fresh Claim Count!', 'error');
        this.freshClaimCount = 0;
      }
    });
  }
  resData: any;
  getCPDClaimList() {
    ...
    this.cpdService.getCPDDraftClaimListOrderBy(this.user.userId, schemeid, schemecategoryid).subscribe(data => {
      this.resData = data;
      if (this.resData.status == 'success') {
        this.claimlist = JSON.parse(this.resData.data);
  ...
  getCPDClaimListCount() {
    ...
    this.getCPDApprovalListCount(this.user.userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid);
  }
  ...
  onAction(claimID: any, transactionID: any, URN: any, transClaimId: any, authorizedcode: any, hospitalcode: any, actualDate: any, caseNo: any, claimNo: any) {
    ...
    this.router.navigate(['/application/cpddraftaction']);
  ...
  searchFilter() {
    ...
    this.cpdService.getCPDClaimListOrderBy(this.user.userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid).subscribe(data => {
      this.getCPDApprovalListCount(this.user.userId, orderValue, fromDate, toDate, authMode, trigger, schemeid, schemecategoryid);
      this.claimlist = data;
      this.allRecordsSize = this.claimlist.length;
      this.showPegi = this.claimlist.length > 0;
      let serFilterParameters = {
      ...
      this.sessionService.encryptSessionData("searchFilterParameters", serFilterParameters);
    });
  }
  ...
  downloadReport(type: any) {
    let SlNo = 1;
    let report = [];
    let heading = [['Sl#', 'Claim No', 'URN', 'Case No', 'Invoice No', 'Patient Name', 'Actual Date of Admission', 'Actual Date of Discharge', 'Allotted Date', 'Action to be Taken By']];
    let months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    if (type == 'excel') {
      let claim: any;
      this.claimlist.forEach(element => {
        claim = {
          ...
          "Allotted Date": this.formatDate(element.allotedDate),
          "Action to be Taken By": this.formatDate(element.takenDate)
        }
        report.push(claim);
        SlNo++;
      });
      TableUtil.exportListToExcel(report, "CPD Draft List", heading);
    } else if (type == 'pdf') {
      if (this.claimlist.length > 0) {
        this.claimlist.forEach(element => {
          ...
          rowData.push(this.formatDate(element.allotedDate));
        ...
        doc.text('CPD Draft List', 220, 30);
        doc.setFontSize(12);
