...
@Component({
  selector: 'app-cpdfreshcase-details',
  templateUrl: './cpdfreshcase-details.component.html',
  styleUrls: ['./cpdfreshcase-details.component.scss'],
})
export class CpdfreshcaseDetailsComponent implements OnInit {
  user: any;
  data: any;
  caseId: any;
  caseNo: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Case for CPD Approval');
  ...
  getCPDCaseDetails() {
    ...
    this.allocateService.getCPDCaseDetails(requestData).subscribe(
      (response) => {
  ...
  submitDetails(event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.allocateService.cpdCaseWiseAction(data).subscribe(
          (data: any) => {
            console.log(data);
            ...
            this.route.navigate(['/application/freshcaseallocation']);
          },
          (error) => {
            console.log(error);
