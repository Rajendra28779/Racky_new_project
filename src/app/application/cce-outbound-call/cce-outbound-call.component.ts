...
@Component({
  selector: 'app-cce-outbound-call',
  templateUrl: './cce-outbound-call.component.html',
  styleUrls: ['./cce-outbound-call.component.scss']
})
export class CceOutboundCallComponent implements OnInit {
  months: string;
  year: number;
  user: any;
  ...
  outBoundData() {
    ...
    this.callCenterExecutiveService.getCceOutBound(action, userId, fromDate, toDate, cceId, hospitalCode, this.pageIn, this.pageEnd,queryStatus,stateCode,distCode).subscribe((data: any) => {
      this.size = data.size;
      ...
      for (var i = 0; i < this.cceOutboundData?.length; i++) {
        ...
        this.cceOutboundData[i].alottedDate=this.convertDateFormat(createDate.alottedDate);
      }
...
togglestatus:boolean=false;
      ...
      if (result.isConfirmed) {
        ...
        this.callCenterExecutiveService.cceSave(formData).subscribe((data: any) => {
          ...
          if (data.statusCode == 200 && data.status == "Success") {
            if(action=='B'){
              data.message='Initial Action Taken Successfully';
            }
  ...
  downloadReport(){
    ...
    for (var i = 0; i < this.cceOutboundData.length; i++) {
      ...
      this.callCenter.dgoQueryRemarks = item.transId ? item.transId : '-NA-';
  ...
  revertToSupervisor(){
  let alternateMobileNo=$('#alternatePhoneno').val();
  let userId = this.user.userId;
  let action ='C';
  if(alternateMobileNo=='' || alternateMobileNo==null || alternateMobileNo==undefined){
    this.swal('Warning', 'Please Enter Alternate Mobile No.', 'warning');
    return;
  }
  if(alternateMobileNo.length<10){
    this.swal('warning','Please Enter valid Mobile No.','warning');
    return;
  }
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      formData.append('userId', userId);
      formData.append('action', action);
      formData.append('alternateNo', alternateMobileNo);
      formData.append('dateOfAdmission', this.cceOutboundDataById.dateOfAdm != undefined ? this.cceOutboundDataById.dateOfAdm : "");
