...
@Component({
  selector: 'app-re-assign-view',
  templateUrl: './re-assign-view.component.html',
  styleUrls: ['./re-assign-view.component.scss']
})
export class ReAssignViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Reassign complete Data');
  ...
  getalldata() {
    this.cceService.getAllReassignViewData(this.user.userId).subscribe((data: any) => {
  ...
  downloadList() {
    ...
    TableUtil.exportListToExcel(
      this.report, 'Patient Reassign Connected Data', this.heading
    );
