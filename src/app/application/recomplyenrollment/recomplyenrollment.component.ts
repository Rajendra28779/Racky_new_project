...
@Component({
  selector: 'app-recomplyenrollment',
  templateUrl: './recomplyenrollment.component.html',
  styleUrls: ['./recomplyenrollment.component.scss']
})
export class RecomplyenrollmentComponent implements OnInit {
  showPegi: boolean;
  currentPage: any;
  pageElement: any;
  txtsearchDate: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Hospital Re-Comply List');
  ...
  getrecomply() {
    ...
    this.dsService.getrecomplylist(this.fromDate, this.toDate, this.urn, this.user.userId).subscribe((data: any) => {
      this.recomplydata = data;
      this.record = this.recomplydata.length;
      if (this.recomplydata.length == 0) {
        this.showPegi = false;
      } else {
        this.showPegi = true;
      }
  ...
  downloadReport(type: any) {
    ...
    if (type == 'excel') {
      let claim: any;
      for (var i = 0; i < this.recomplydata.length; i++) {
        claim = this.recomplydata[i];
        this.sno = [];
        this.sno.Slno = i + 1;
      ...
      TableUtil.exportListToExcelWithFilterforhospitals(this.report, "Hospital Re-Comply List", this.heading, filter1);
    } else if (type == 'pdf') {
      if (this.recomplydata.length == 0) {
      ...
      let SlNo = 1;
      this.recomplydata.forEach(element => {
        let rowData = [];
        rowData.push(SlNo);
      ...
      doc.text('Hospital_Re-Comply_List', 100, 26);
      doc.setLineWidth(0.7);
      doc.line(100, 27, 130, 27);
      autoTable(doc, {
      ...
      doc.save('Hospital_Re-Comply_List.pdf');
    }
  }
  onclaim(data: any) {
    let state = {
