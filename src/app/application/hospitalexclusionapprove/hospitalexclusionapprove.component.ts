...
@Component({
  selector: 'app-hospitalexclusionapprove',
  templateUrl: './hospitalexclusionapprove.component.html',
  styleUrls: ['./hospitalexclusionapprove.component.scss']
})
export class HospitalexclusionapproveComponent implements OnInit {
  user: any;
  list: any = [];
  txtsearchDate: any;
  txtsearch: any;
  ...
  getappliedlist() {
    if (this.user.groupId == 4) {
      this.config.getappliedlistsna(this.user.userId).subscribe((data: any) => {
        this.list = data;
      ...
      this.config.getappliedlistadmin().subscribe((data: any) => {
        this.list = data;
        this.totalcount = this.list.length;
        if (this.totalcount > 0) {
          this.showPegi = true;
  ...
  onaction(item: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.config.approveofexclusion(item.hospital?.hospitalCode, item.bskyuserid, this.user.userId, ip).subscribe((data: any) => {
          if (data.status == 200) {
  ...
  heading = [['Sl#', 'CPD Name', 'Hospital Name', 'Hospital Code', 'State Name', 'District Name', 'Tagged From', 'Applied On']];
  downloadList(no: any) {
    ...
    for (var i = 0; i < this.list.length; i++) {
      ...
      this.sno.tag = this.convertDate1(sna.taggeddate);
    ...
    if (no == 1) {
      ...
      doc.setFontSize(20);
      doc.text("Removal of Restriction", 100, 15);
      doc.setFontSize(13);
      ...
      doc.save('Removal of Restriction.pdf');
    }
  }
