...
@Component({
  selector: 'app-supervisor-cce-view',
  templateUrl: './supervisor-cce-view.component.html',
  styleUrls: ['./supervisor-cce-view.component.scss']
})
export class SupervisorCceViewComponent implements OnInit {
  months: string;
  year: number;
  user: any;
  ...
  heading = [['Sl No', 'Case No', 'URN', 'Patient Name', 'District Name', 'Block Name', 'Panchayat Name', 'Village Name', 'DateOf Admission', 'Total Amount Blocked', 'Hospital District', 'Hospital Name', 'Procedure Name', 'Package Name', 'Patient PhoneNo', 'Alternate PhoneNo', 'Invoice No', 'MobileNo Active Status', 'Call Response', 'Feedback App Login Id', 'Data Receive Date', 'Calling Date', 'Dialed Count', 'Are you receiving cashless treatment under GJAY (ଆପଣ ଜି.ଜେ.ଏ.ୱାଇ ଅଧୀନରେ ନଗଦବିହୀନ ଚିକିତ୍ସା ଗ୍ରହଣ କରୁଛନ୍ତି କି)', 'Are you satisfied with service? (ଆପଣ ସେବାରେ ସନ୍ତୁଷ୍ଟ କି?)', 'Whether you have paid any extra money for treatment under GJAY (ଆପଣ ଜି.ଜେ.ଏ.ୱାଇ ଅଧୀନରେ ଚିକିତ୍ସା ପାଇଁ କୌଣସି ଅତିରିକ୍ତ ଟଙ୍କା ଦେଇଛନ୍ତି କି)', 'Did Swasthya Mitra facilitate your care (ସ୍ୱାସ୍ଥ୍ୟ ମିତ୍ର ଆପଣଙ୍କ ଯତ୍ନକୁ ସୁଗମ କରିଥିଲେ କି?', 'CCE Remark', 'Remark']];

  ngOnInit(): void {
    this.headerService.setTitle('Call Details View');
  ...
  outBoundData() {
    ...
    this.callCenterExecutiveService.getSupervisorCallCenterData(action, userId, fromDate, toDate, cceId, hospitalCode, cceUserId, this.pageIn, this.pageEnd,stateCode,distCode).subscribe((data: any) => {
      this.size = data.size;
  ...
  downloadReport() {
    ...
    for (var i = 0; i < this.cceOutboundData.length; i++) {
      ...
      this.callCenter.dataReceivedDate = item.alottedDate ? item.alottedDate : '-NA-';
      this.callCenter.callingDate = this.datepipe.transform(item.createdOn, 'dd-MMM-yyyy');
      // this.callCenter.repeatCheck=item.feedbackLoginId ? item.feedbackLoginId : '-NA-' ;
      ...
      this.callCenter.cceremark = item.executiveRemarks ? item.executiveRemarks : '-NA-';
      this.callCenter.remark = item.reAssignRemark ? item.reAssignRemark : '-NA-';

      this.report.push(this.callCenter);
      console.log(this.report);
      console.log(this.callCenter);
    }
    TableUtil.exportListToExcel(this.report, "Re-assign Out Bound Call Details", this.heading);
  }
