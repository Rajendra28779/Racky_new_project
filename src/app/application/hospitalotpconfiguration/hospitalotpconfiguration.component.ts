...
@Component({
  selector: 'app-hospitalotpconfiguration',
  templateUrl: './hospitalotpconfiguration.component.html',
  styleUrls: ['./hospitalotpconfiguration.component.scss']
})
export class HospitalotpconfigurationComponent implements OnInit {
  user: any;
  stateList: any=[];
  districtList: any=[];
  txtsearchDate:any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle("Hospital Patient OTP Configuration");
  ...
  Search(){
    ...
    this.hospitaService.hospitallistforotpconfigure(this.state, this.dist,this.user.userId,this.otpreq).subscribe((data:any)=>{
      this.list=data.data;
  ...
  Submit(){
    Swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          this.hospitaService.submithospitallistforotpconfigure(object).subscribe((data:any)=>{
            if(data.status==200){
  ...
  heading = [['Sl#', 'Hospital Code','Hospital Name','Hospital MobileNo', 'Hospital emailId','Patient OTP Required']];
    ...
    if(no==1){
      ...
      filter.push([['State Name', this.statename]]);
        ...
        TableUtil.exportListToExcelWithFilter(
          this.report,
          'Hospital Patient OTP Configuration',
          this.heading,filter
        );
    }else{
      ...
      doc.setFontSize(20);
      // doc.text(" ", 5, 5);
      doc.text("Hospital Patient OTP Configuration", 60, 15);
      doc.setFontSize(12);
      ...
      doc.text('GeneratedBy :- '+generatedBy,110,33);
            ...
            doc.save('Hospital Patient OTP Configuration.pdf');
    }
  }
