...
@Component({
  selector: 'app-dgoinitial-take-action',
  templateUrl: './dgoinitial-take-action.component.html',
  styleUrls: ['./dgoinitial-take-action.component.scss']
})
export class DGOInitialTakeActionComponent implements OnInit {
  user: any;
  pageElement: any;
  currentPage: number;
  months: string;
  ...
  outBoundData() {
    ...
    this.dgoCallCenterService.getITADgoCallCenterData(userId, action, fromDate, toDate, hospitalCode, stateCode, distCode).subscribe((data: any) => {
      console.log(data)
      this.size = data.size;
  ...
  downloadReport() {
    ...
    for (var i = 0; i < this.cceOutboundData.length; i++) {
      ...
      // this.callCenter.goQueryRemarks = item.dcRemarks ? item.goQueryRemarks : '-NA-'
    ...
    TableUtil.exportListToExcel(this.report, "DGO Initial Take Action", this.heading);
