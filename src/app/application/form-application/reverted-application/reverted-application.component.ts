...
@Component({
  selector: 'app-reverted-application',
  templateUrl: './reverted-application.component.html',
  styleUrls: ['./reverted-application.component.scss']
})
export class RevertedApplicationComponent implements OnInit {
  ...
  utillist: any;
  messaageslist: any;
  jsonurl = 'assets/js/_configs/revertedApplications.config.json';
  formsList: any;
  isFlag = true;
  POSTS: any;
  page: number = 1;
  ...
  ngOnInit(): void {
    this.headerService.setTitle("Sent for Resubmission");
 ...
 getApplList(formID: any) {
    let param = { itemId: formID, pendingAt: this.userRole, pageType: 'rev', profileId:0 ,serviceId:0,
  ...
  searchData(){
    ...
    this.param = { itemId: this.formID, pendingAt: this.userRole, pageType: 'rev', profileId:0,serviceId:0,
