...
@Component({
  selector: 'app-claim-processed-action',
  templateUrl: './claim-processed-action.component.html',
  styleUrls: ['./claim-processed-action.component.scss'],
})
export class ClaimProcessedActionComponent implements OnInit {
  childmessage: any;
  claimDetails: any;
  allremarks: any;
  txnId: any;
  ...
  getClaimProcessedDetails() {
    this.snoService.claimProcessedDetails(this.txnId).subscribe(
      (data: any) => {
        let resData = data;
        if (resData.status == 'success') {
          let details = JSON.parse(resData.details);
          this.claimDetails = details.actionData;
          this.getsnamortalitystatus(this.claimDetails.CLAIMID);//forget sna mortalitystatus
  ...
  viewAllDocument(
    ...
    console.log("intra" + intraSurgery);
  ...
  resuestedData:any;
  submitClaimAction(){
    this.snoService.saveClaimProcessedDetails(this.resuestedData).subscribe(
      (data: any) => {
