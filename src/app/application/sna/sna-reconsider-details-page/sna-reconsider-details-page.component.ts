...
@Component({
  selector: 'app-sna-reconsider-details-page',
  templateUrl: './sna-reconsider-details-page.component.html',
  styleUrls: ['./sna-reconsider-details-page.component.scss']
})
export class SnaReconsiderDetailsPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.headerService.setTitle('SNA Re-Consider');
    $('#appealDisposal').hide();
    ...
    this.snoService.getOldClaimResettlementById(this.txnId).subscribe(
      (data: any) => {
        ...
        if (resData.status == 'success') {
          ...
          for (let i = 0; i < this.allremarks.length; i++) {
            let data = this.allremarks[i];
            if(this.claimDetails.CLAIMS_TATUS == 1 && data.id == 1){
              this.allremarks.splice(i, 1);
              this.isApproved = false;
              break;
            } else if(this.claimDetails.CLAIMS_TATUS == 2 && data.id == 58){
              this.allremarks.splice(i,1);
              this.isReject = false;
            }
          }
  ...
  viewAllDocument(dischargeSlip, additionDoc, additionalDoc1, additionalDoc2,investigationdocs,investigationdocs2, preSurgery, postSurgery, patientPic, intraSurgery, specimenRemoval) {
    ...
    console.log("investigationdocs: " + investigationdocs);
    console.log("investigationdocs2: " + investigationdocs2);
