...
@Component({
  selector: 'app-partial-claim-approval-details',
  templateUrl: './partial-claim-approval-details.component.html',
  styleUrls: ['./partial-claim-approval-details.component.scss'],
})
export class PartialClaimApprovalDetailsComponent implements OnInit {
  childmessage: any;
  claimDetails: any;
  allremarks: any;
  txnId: any;
  ...
  ngOnInit(): void {
    ...
    this.headerService.setTitle('Partial Claim');
  ...
  submitDetails(event, claimId: any) {
    ...
    if (event.target.id == 'Approve') {
      if (Number(this.amount) == 0) {
        this.swal('Info', 'Partial Claim Approved amount should not be zero', 'info');
        return;
      }
    }
    ...
    if (this.amount == '' || this.amount == null) {
      this.swal('', 'Partial Claim Amount should not be left blank', 'error');
            ...
            if (data.status == 'Success') {
              ...
              if(this.data.status){
                if(this.data.status==2){
                  this.route.navigate(['/application/partialclaimhosnoncom']);
                }else if(this.data.status==3){
                  this.route.navigate(['/application/partialclaimdcnoncom']);
                }else{
