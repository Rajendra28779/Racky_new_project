...
@Component({
  selector: 'app-non-compliance-query-snato-sna',
  templateUrl: './non-compliance-query-snato-sna.component.html',
  styleUrls: ['./non-compliance-query-snato-sna.component.scss']
})

export class NonComplianceQuerySNAToSNAComponent implements OnInit {
  ...
  ngOnInit(): void {
    // dynamic title
    this.getSchemeData();
    this.headerService.setTitle('Non Compliance Query By SNA');
  ...
  onAction(claimid, urn: any) {
    ...
    // };
    // localStorage.setItem('actionData1', JSON.stringify(state));
    // sessionStorage.setItem('currentPageNum', JSON.stringify(this.currentPage));
    // this.route.navigate(['/application/NonComplianceQuerySNAToSNA/action']);
  ...
  getSnoClaimDetails() {
    ...
    this.pendingService.getSNASystemRejectedataForSNA(requestData).subscribe(
      (response) => {
  ...
  heading = [['Sl#', 'URN', 'Claim No', 'Patient Name', 'Phone No', 'Hospital Details', 'Invoice No', 'Package ID', 'Actual Date of Admission', 'Actual Date of Discharge', 'Hospital Claim Amount (₹)', 'Last Date Of SNA Query', 'CPD Mortality', 'Hospital Mortality']];
  ...
  downloadReport() {
    ...
    // TableUtil.exportListToExcel(this.report, "Non Compliance Query By SNA List", this.heading);
    ...
    TableUtil.exportListToExcelWithFilter(this.report, 'Non Compliance Query By SNA List', this.heading, filter);
  }
  ...
  downloadPdf() {
    if (this.snoclaimlist.length == 0) {
      ...
      doc.text("Non Compliance Query By SNA List", 110, 70);
      let col = [['Sl#', 'URN', 'Claim No', 'Patient Name', 'phone No', 'Hospital Details', 'Invoice No', 'Package ID', 'Actual Date of Admission', 'Actual Date of Discharge', 'Hospital Claim Amount', 'Last Date Of SNA Query', 'CPD Mortality', 'Hospital Mortality']];
      let rows = [];
      let claim: any;
      for (let i = 0; i < this.snoclaimlist.length; i++) {
        claim = this.snoclaimlist[i];
      ...
      autoTable(doc, {
        ...
        columnStyles: { 0: { cellWidth: 10 }, 1: { cellWidth: 20 }, 2: { cellWidth: 20 }, 3: { cellWidth: 20 }, 4: { cellWidth: 20 }, 5: { cellWidth: 30 }, 6: { cellWidth: 20 }, 7: { cellWidth: 20 }, 8: { cellWidth: 30 }, 9: { cellWidth: 20 }, 10: { cellWidth: 30 }, 11: { cellWidth: 20 }, 12: { cellWidth: 15 }, 13: { cellWidth: 15 } }
      });
      doc.save('Non Compliance Query By SNA List.pdf');
    }
  }
