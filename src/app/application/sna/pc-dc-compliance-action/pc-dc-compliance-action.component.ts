...
@Component({
  selector: 'app-pc-dc-compliance-action',
  templateUrl: './pc-dc-compliance-action.component.html',
  styleUrls: ['./pc-dc-compliance-action.component.scss'],
})
export class PcDcComplianceActionComponent implements OnInit {
  childmessage: any;
  claimDetails: any;
  allremarks: any;
  textRemak: any;
  ...
  getSnoClaimDetailsById() {
    this.snoService.getPCDcAprvClaimById(this.txnId).subscribe(
      (data: any) => {
        let resData = data;
  ...
  submitDetails(event: any, claimId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.snoService.savePartialClaimOf(data).subscribe(
          (data: any) => {
            if (data.status == 'Success') {
              ...
              this.route.navigate(['/application/partialdccompliance']);
