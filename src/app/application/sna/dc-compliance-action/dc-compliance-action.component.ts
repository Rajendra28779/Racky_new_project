...
@Component({
  selector: 'app-dc-compliance-action',
  templateUrl: './dc-compliance-action.component.html',
  styleUrls: ['./dc-compliance-action.component.scss']
})
export class DcComplianceActionComponent implements OnInit {
  childmessage: any;
  claimDetails: any;
  allremarks: any;
  textRemak: any;
  ...
  getSnoClaimDetailsById() {
    this.snoService.getSnoDcAprvClaimListById(this.txnId).subscribe(
      (data: any) => {
        let resData = data;
  ...
  submitDetails(event: any, claimId: any) {
    ...
    let investigationdocs1 = this.claimDetails.INVESTIGATIONDOC;
    let investigationdocs2 = this.claimDetails.INVESTIGATIONDOC2;
    ...
    if (investigationdocs1 == undefined) {
      investigationdocs1 = null;
    }
    if (investigationdocs2 == undefined) {
      investigationdocs2 = null;
    }
    this.requestedData = {
      claimId: claimId,
      userId: userId,
      ...
      investigationdocs1: investigationdocs1,
      investigationdocs2: investigationdocs2,
  ...
  documentstatus: any;
  files: any = [];
  viewAllDocument(dischargeSlip, additionDoc, additionalDoc1, additionalDoc2, investigationdocs, investigationdocs2, preSurgery, postSurgery, patientPic, intraSurgery, specimenRemoval, mortalityauditreport) {
    this.files = [];
    this.documnetname = [];
  ...
  submitClaimAction(){
    this.snoService.saveSnoDetailsOfDCAprv(this.requestedData).subscribe(
      (data: any) => {
        if (data.status == 'Success') {
          ...
          if (this.treat == 'Y') {
            ...
            this.route.navigate(['/application/dcCompliance']);
          }
