...
declare let $: any;
@Component({
  selector: 'app-partial-claim-sno-reapproval',
  templateUrl: './partial-claim-sno-reapproval.component.html',
  styleUrls: ['./partial-claim-sno-reapproval.component.scss']
})
export class PartialClaimSnoReapprovalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.headerService.setTitle('Partial Claim Resettlement');
  ...
  onclaim(id: any, urn: any) {
    ...
    this.route.navigate(['/application/partialclaimreapproval/action']);
  }
  ...
  heading = [
    ['Sl No.', 'URN', 'Patient Name', 'Case Number', 'Invoice Number',
     ...
     'Discharge Date', 'Actual Date Of Discharge',
     'Claim Amount', 'Approved Amount', 'Partial Claim Amount']
  ];
  report:any=[];
  downloadReport(no: any) {
    ...
    for (let i = 0; i < this.claimlist.length; i++) {
      ...
      sno.push(claim.invoiceno); // Invoice Number
      ...
      // sno.push(claim.actualdateofadmission); // Actual Date Of Admission
      sno.push(claim.dateOfDischarge); // Discharge Date
      sno.push(claim.actualdateofdischarge); // Actual Date Of Discharge
      // sno.push(claim.claimRaiseby); // Claim Raised By
    ...
    if (no == 1) {
      ...
      TableUtil.exportListToExcelWithFilter(
        this.report,
        'Partial_Claims_SNA_Re-Settelment',
        this.heading, filter
      );
    } else {
      ...
      doc.setFontSize(20);
      doc.text("Partial Claims SNA Settelment", 100, 15);
      doc.setFontSize(13);
      ...
      doc.save('Partial_Claims_SNA_Settelment.pdf');
    }
  }
