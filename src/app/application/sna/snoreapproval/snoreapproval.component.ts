...
declare let $: any;
@Component({
  selector: 'app-snoreapproval',
  templateUrl: './snoreapproval.component.html',
  styleUrls: ['./snoreapproval.component.scss'],
})
export class SnoreapprovalComponent implements OnInit {
  childmessage: any;
  user: any;
  txtsearchDate: any;
  statelist: Array<any> = [];
  ...
  getSnoClaimDetails() {
    ...
    this.snoService.getSnoReAprovClaimList(requestData).subscribe(
      (data) => {
  ...
  downloadReport() {
    ...
    TableUtil.exportListToExcelWithFilter(this.report, 'SNA Re-Settlement List', this.heading, filter);
  }
  // GetDate(str) {
  //   var arr = str.split("-");
  ...
  downloadPdf() {
    if (this.snoclaimlist.length == 0) {
      ...
      doc.text("SNA Re-Settlement List", 110, 70);
      ...
      for (var i = 0; i < this.snoclaimlist.length; i++) {
        claim = this.snoclaimlist[i];
        var temp = [i + 1, claim.urn, claim.claimNo, claim.patientName, claim.phone, claim.hospitalName + '(' + claim.hospitalCode + ')', claim.invoiceNumber, claim.packageCode, this.convertStringToDate(claim.actualDateOfAdmission), this.convertStringToDate(claim.actualDateOfDischarge), this.convertCurrency(claim.currentTotalAmount),
      ...
      doc.save('SNA Re-Settlement List.pdf');
    }
  ...
  getPackageHeader() {
    this.hospitalService.getallPackageHeaders().subscribe((data: any) => {
      ...
      // if (this.dataRequest.procedure != null ||this.dataRequest.procedure != undefined || this.dataRequest.procedure != '') {
      //   this.procedure = this.dataRequest.procedure;
      //   this.getPackageName(this.procedure);
      // } else {
      //   this.getSnoClaimDetails();
