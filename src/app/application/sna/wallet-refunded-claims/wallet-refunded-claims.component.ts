...
@Component({
  selector: 'app-wallet-refunded-claims',
  templateUrl: './wallet-refunded-claims.component.html',
  styleUrls: ['./wallet-refunded-claims.component.scss']
})
export class WalletRefundedClaimsComponent implements OnInit {
  statelist: any = [];
  user: any;
  stateCode: any;
  userId: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Wallet Refunded Claims');
  ...
  getWalletRefundedDetails() {
    ...
    this.paymentfreezeService.getrefundlist(requestData).subscribe(
      (data) => {
        this.resData = data;
  ...
  getActionDetails(claimid, urn: any, refundedBy) {
    ...
    localStorage.setItem('refundedBy', refundedBy);
  ...
  downloadReport(type) {
    ...
    if (type == 'xcl') {
      ...
      TableUtil.exportListToExcelWithFilter(this.report, 'Wallet Refunded Claims', this.heading, filter);
    } else if (type == 'pdf') {
      ...
      doc.save('Bsky_Wallet Refunded Claims.pdf');
    }
  }
