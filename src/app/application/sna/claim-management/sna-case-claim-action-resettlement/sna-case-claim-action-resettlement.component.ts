...
@Component({
  selector: 'app-sna-case-claim-action-resettlement',
  templateUrl: './sna-case-claim-action-resettlement.component.html',
  styleUrls: ['./sna-case-claim-action-resettlement.component.scss']
})
export class SnaCaseClaimActionResettlementComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Case for SNA Resettlement');
  ...
  getSNACaseDetails() {
    ...
    this.snoClaimApprovalService.getSnaCaseDetailsForresettlement(requestData).subscribe(
      (response) => {
  ...
  submitDetails(event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.snoClaimApprovalService.SnaCaseWiseActionForResettlement(data).subscribe(
          (data: any) => {
            console.log(data);
            ...
            this.route.navigate(['/application/snaCaseResettlement']);
          },
          (error) => {
            console.log(error);
