...
declare let $: any;
@Component({
  selector: 'app-fresh-claim-details',
  templateUrl: './fresh-claim-details.component.html',
  styleUrls: ['./fresh-claim-details.component.scss']
})
export class FreshClaimDetailsComponent implements OnInit {
  show: boolean = false;
  user: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('SNA Approval Case');
  ...
  onAction(id: any, urn: any, caseNo: any) {
    ...
    this.route.navigate(['/application/cpdApprovedDeatailsCase']);
  }

  getClaimCPDApprovalCase(reqData:any){
    reqData= this.encryptedService.encryptRequest(reqData);
    this.snoClaimApprovalService.getCPDApprovalCase(reqData).subscribe(
      (data: any) => {
        if(data.length >0){
  ...
  getPackageName(code:any){

  }
  selectEvent1(event:any){
