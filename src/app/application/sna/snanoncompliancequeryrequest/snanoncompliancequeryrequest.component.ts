...
@Component({
  selector: 'app-snanoncompliancequeryrequest',
  templateUrl: './snanoncompliancequeryrequest.component.html',
  styleUrls: ['./snanoncompliancequeryrequest.component.scss']
})
export class SnanoncompliancequeryrequestComponent implements OnInit {
  totalCount: any;
  ...
  schemeidvalue: any = 1
  schemeName: any
  constructor(private sessionService: SessionStorageService, public headerService: HeaderService, public snoService: SnoCLaimDetailsService,
    public rejectedrequest: RejectRequestService, public route: Router, private encryptionService: EncryptionService, public packageDetailsMasterService: PackageDetailsMasterService, private LoginServ: ClaimRaiseServiceService) { }
  ...
  getRejectedRequestList() {
    ...
    this.flag = "SNA";
    let fromDate = $('#date9').val();
    let toDate = $('#date10').val();
    ...
    this.rejectedrequest.getNonComplianceRequestList(userId, fromDate, toDate, stateCode1, distCode1, hospitalCode, this.flag,schemeid,schemecategoryid).subscribe(data => {
      // console.log(data);
  ...
  onAction(id: any, urn: any, packageCode: any) {
    ...
    localStorage.setItem("noncompliancequeryData", JSON.stringify(state));
    this.route.navigate(['/application/NoncomplianceQueryRequest/action']);
  }
  ...
  heading = [['Sl#', 'URN', 'Claim No', 'Patient Name', 'Package ID', 'Requested On', 'Last Query On', '	Amount (₹)']];
  ...
  downloadReport() {
    ...
    for (var i = 0; i < this.rejectRequestList.length; i++) {
      ...
      this.sno.lastQueryOn = this.convertDate(claim.lastQueryOn);
      this.sno.amount = this.convertCurrency(claim.currentTotalAmount);

      this.report.push(this.sno);
    }
    TableUtil.exportListToExcel(this.report, "SNA Non compliance Query List", this.heading);
  }
  GetDate(str) {
    var arr = str.split("-");
