...
@Component({
  selector: 'app-hospital-auth-claim-qry-cpd',
  templateUrl: './hospital-auth-claim-qry-cpd.component.html',
  styleUrls: ['./hospital-auth-claim-qry-cpd.component.scss']
})
export class HospitalAuthClaimQryCPDComponent implements OnInit {
  hospitalcode: any='';
  bskyUserId: any;
  ...
  ngOnInit(): void {
    ...
    this.headerService.setTitle("Claim Query By CPD");
    ...
    if(localStorage.getItem('hospitalNav') != 'N'){
      sessionStorage.removeItem('claimData');
    }
    // this.dataRequest = JSON.parse(sessionStorage.getItem('claimData'));
  ...
  Search(){
    ...
    this.auhtorityReport.SearchClaimQueryByCPDList(this.fromDate,this.toDate,this.type,this.hospitalcode,userid).subscribe(
      (response) => {
        console.log(response);
          this.list=response;
  ...
  onclaim(claimID: any) {
   ...
   localStorage.setItem('hospitalNav', 'N');
   ...
   this.route.navigate(['/application/claimsqueriedbyCPDdetails/Action']);
 }
 modalClose() {
  $("#appealDisposal").hide();
}
...
downloadReport(type: any){
  ...
  if(type == 'excel'){
    for(var i=0;i<this.list.length;i++) {
      ...
      this.sno.Amount =this.convertCurrency(claim.cpdapprovedamount);
      this.report.push(this.sno);
    }
   TableUtil.exportListToExcel(this.report, "Claims Queried by CPD", this.heading);
}else if(type == 'pdf'){
  let SlNo = 1;
  ...
  doc.text('Claims Queried by CPD',150,25);
