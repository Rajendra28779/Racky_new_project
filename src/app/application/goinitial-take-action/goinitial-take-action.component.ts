...
@Component({
  selector: 'app-goinitial-take-action',
  templateUrl: './goinitial-take-action.component.html',
  styleUrls: ['./goinitial-take-action.component.scss']
})
export class GOInitialTakeActionComponent implements OnInit {
  user: any;
  pageElement: any;
  currentPage: number;
  months: string;
  ...
  heading = [['Sl No', 'URN', 'Patient Name', 'Patient PhoneNo', 'State Name', 'District Name', 'Block Name', 'Panchayat Name', 'Village Name', 'Call Response', 'MobileNo Active Status', 'Hospital District', 'Hospital Name', 'Procedure Name', 'Package Name', 'Invoice No', 'CCE Alloted Date', 'Alternate PhoneNo', 'ID', 'CCE Remarks', 'DC ITA Remarks','DGO ITA Remarks', 'DateOf Admission', 'Total Amount Blocked', 'DGO Action Date', 'Are you receiving cashless treatment under GJAY', 'Are you satisfied with service? ', 'Whether you have paid any extra money for treatment under GJAY', 'Did Swasthya Mitra facilitate your care?']];
  ngOnInit(): void {
  ...
  outBoundData() {
    ...
    this.callCenterExecutiveService.getGOITAData(userId,fromDate, toDate, stateCode, distCode, hospitalCode, action).subscribe((data: any) => {
      console.log(data)
      this.size = data.size;
  ...
  saveGOCall(cceId:any,event: any) {
    ...
    if (remarks == undefined || remarks == null || remarks == "") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.callCenterExecutiveService.addGoRemark(cceId, event, remarks, userId).subscribe((data: any) => {
            // this.res = data;
            if (data.status == "Success") {
              this.swal("Success", 'Initial Action Taken Successfully', "success");
              this.onSearch();
  ...
  downloadReport() {
    ...
    for (var i = 0; i < this.cceOutboundData.length; i++) {
      ...
      this.callCenter.goQueryRemarks = item.dgoRemarks ? item.dgoRemarks : '-NA-';
      ...
      this.callCenter.dgoAllotedDate = this.datepipe.transform(item.dgoAction, 'dd-MMM-yyyy');
    ...
    TableUtil.exportListToExcel(this.report, "GO Initial Take Action", this.heading);
  }
