...
@Component({
  selector: 'app-surveyquestionmappingview',
  templateUrl: './surveyquestionmappingview.component.html',
  styleUrls: ['./surveyquestionmappingview.component.scss']
})
export class SurveyquestionmappingviewComponent implements OnInit {
  childmessage: any;
  txtsearchDate:any;
  showPegi:any;
  pageElement:any;
  ...
  getallmappinglist(){
    this.surveyserv.getallsurveyquestionmappinglist(this.surveyid).subscribe((data:any)=>{
      console.log(data);
      if(data.status==200){
        this.list=data.data;
  ...
  report: any = [];
  sno: any = [];
  heading = [['Sl#', 'Survey Name','Question','Created By', 'Created On']];
  downloadList(no:any){
    ...
    if(no==1){
      ...
      doc.setFontSize(20);
      doc.text("Survey Question Mapping List", 80, 15);
      doc.setFontSize(13);
      ...
      doc.text('GeneratedBy :- '+generatedBy,15,41);
            ...
            doc.save('Survey_Question_Mapping_List.pdf');
    }
  }
}
