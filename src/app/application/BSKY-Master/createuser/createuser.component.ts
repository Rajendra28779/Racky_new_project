...
export class CreateuserComponent implements OnInit {
  ...
  constructor(public fb: FormBuilder, private route: Router, public headerService: HeaderService, private userService: UsercreateService, private encryptionService: EncryptionService, private sessionService: SessionStorageService) {
  ...
  ngOnInit(): void {
    ...
    if (this.bId) {
      ...
      this.userService.getbyid(this.bId.bskyUserId).subscribe(data => {
        ...
        this.updatelist.status == 0 ? this.activeStatus = true : this.activeStatus = false;
    ...
    this.userService.getGroupList().subscribe(
      (response: any) => {
        ...
        for (var i = 0; i < groups.length; i++) {
          var g = groups[i];
          if (g.typeId != 3 && g.typeId != 5 && g.typeId != 16 && g.typeId != 25) {
  ...
  SubmitCreate() {
    ...
    if (fullname.length < 3) {
      $("#fullname").focus();
      this.swal("Info", "Fullname must be more than 3 character", 'info');
      return;
    }
      ...
      if (result.isConfirmed) {
        this.userService.saveData(this.addForm.value).subscribe(data => {
          if (data == 1) {
            ...
            this.route.navigate(['application/viewuser']);
            this.addForm.reset();
            $("#groupId").val("");
            $("#stateId").val("");
            $("#districtId").val("");
  ...
  updategroup(items: any) {
    ...
    this.addForm.value.createdUserName = createon;
    this.addForm.value.date = date;
    this.addForm.value.status = this.activeStatus == true ? "0" : "1";
    Swal.fire({
      title: 'Are you sure?',
  ...
  validateName() {
    ...
    if (fullName.length < 3) {
      $("#fullname").focus();
      this.swal("Info", "Full Name must be more than 3 character", 'info');
      return;
    }
  }

  checkUserName() {
    let userName = $('#username').val().toString().trim().toLowerCase();
