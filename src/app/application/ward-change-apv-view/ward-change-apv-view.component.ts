...
@Component({
  selector: 'app-ward-change-apv-view',
  templateUrl: './ward-change-apv-view.component.html',
  styleUrls: ['./ward-change-apv-view.component.scss']
})
export class WardChangeApvViewComponent implements OnInit {
  wardchangeList: any = [];
  txtsearchDate: any;
  pageElement: any;
  currentPage: any;
  ...
  downloadList(no:any){
    ...
    for (let i = 0; i < this.wardchangeList.length; i++) {
        ...
        row.push(data.requestDate || 'N/A'); // Requested Date
    ...
    if (no === 1) {
        ...
        TableUtil.exportListToExcelWithFilter(report, 'Ward Change Action_Taken List', heading, filter);
    } else {
        // Export as PDF
        if (report.length === 0) {
        ...
        doc.setFontSize(20);
        doc.text("Ward Change Action Taken", 100, 15);
        doc.setFontSize(12);
        ...
        doc.save('Ward_Change_Action_Taken_List.pdf');
    }
  }
