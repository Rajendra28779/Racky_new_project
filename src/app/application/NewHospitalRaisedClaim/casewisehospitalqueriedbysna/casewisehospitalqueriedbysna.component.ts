...
@Component({
  selector: 'app-casewisehospitalqueriedbysna',
  templateUrl: './casewisehospitalqueriedbysna.component.html',
  styleUrls: ['./casewisehospitalqueriedbysna.component.scss']
})
export class CasewisehospitalqueriedbysnaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getcasewisequeriedsnadtls();
  }
  ...
  getcasewisequeriedsnadtls() {
    ...
    this.queryByCpdService.getcasewisequeriedsnadtls(hospitalCode, this.fromDate, this.toDate, urn, caseno, schemeid, schemecategoryid).subscribe(
      (response: any) => {
  ...
  heading = [['Sl#', 'URN', 'Case Number', 'Patient Name', 'Date of Admission', 'SNA Query Date', 'Remarks', 'Amount']];
  downloadReport(type:any){
    ...
    if (type == 'excel') {
      ...
      for (var i = 0; i < this.querylist.length; i++) {
        ...
        this.sno.snaquerydate = claim.snaquerydate;
      ...
      TableUtil.exportListToExcelWithFilterforhospitals(this.report, "CaseWise Queried by SNA", this.heading, filter1);
    } else if (type == 'pdf') {
      ...
      this.querylist.forEach(element => {
        ...
        rowData.push(element.snaquerydate);
      ...
      doc.text('CaseWise Queried by SNA', 100, 42);
      ...
      doc.save('CaseWise_Queried_by_SNA.pdf');
    }
  }
  ...
  onclaim(caseid: any) {
    ...
    localStorage.setItem("actionDataforsna", JSON.stringify(state));
    localStorage.setItem("currentPageNumber", JSON.stringify(this.currentPage));
    this.router.navigate(['/application/CasewiseQueriedbysnasubmit']);
  }
}
