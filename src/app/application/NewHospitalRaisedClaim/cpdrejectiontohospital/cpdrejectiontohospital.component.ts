...
@Component({
  selector: 'app-cpdrejectiontohospital',
  templateUrl: './cpdrejectiontohospital.component.html',
  styleUrls: ['./cpdrejectiontohospital.component.scss']
})
export class CpdrejectiontohospitalComponent implements OnInit {
  schemeidvalue: any
  schemeName: any
  currentPage: any;
  pageElement: any;
  currentPagenNum: any;
  getcpdrejectedList: any = [];
  showPegi: boolean;
  txtsearchDate: any;
  user: any;
  hospitalCode: any;
  ...
  ngOnInit(): void {
    this.headerService.setTitle('Casewise CPD Rejected To Hospital');
    this.currentPage = 1;
    this.pageElement = 50;
    this.getSchemeData();
  ...
  getcpdrejectedstatus() {
    ...
    this.LoginServ.getcpdrejectedList(this.hospitalCode, this.fromDate, this.toDate, this.urn, this.caseNo, this.schemeId, this.schemeCategoryId)
      .subscribe({
        next: (response: any) => {
          if (response && response.status === 200 && response.data) {
            this.getcpdrejectedList = response.data;
            this.record = this.getcpdrejectedList.length;
  ...
  heading = [['Sl#', 'URN', 'Case Number', 'Patient Name', 'Date of Admission', 'Cpd Rejected Date','Remark','Description', 'Amount']];
  downloadReport(type: any) {
    this.report = [];
    if (type == 'excel') {
      let claim: any;
      for (var i = 0; i < this.getcpdrejectedList.length; i++) {
        claim = this.getcpdrejectedList[i];
        this.sno = [];
        this.sno.Slno = i + 1;
        ...
        this.sno.cpdrejecteddate = claim.cpdactiondate;
      ...
      TableUtil.exportListToExcelWithFilterforhospitals(this.report, "Casewise_cpd_rejected_to_hospital", this.heading, filter1);
    } else if (type == 'pdf') {
      if (this.getcpdrejectedList.length == 0) {
        this.swal('', 'No Data Found', 'info');
        return;
      }
      let SlNo = 1;
      this.getcpdrejectedList.forEach(element => {
        let rowData = [];
        rowData.push(SlNo++);
      ...
      doc.text('Casewise cpd rejected to hospital', 100, 42);
      doc.setLineWidth(0.7);
      doc.line(100, 43, 152, 43);
      autoTable(doc, {
      ...
      doc.save('Casewise_cpd_rejected_to_hospital.pdf');
    }
  }
