...
@Component({
  selector: 'app-noncompliancesnaquerytohospital',
  templateUrl: './noncompliancesnaquerytohospital.component.html',
  styleUrls: ['./noncompliancesnaquerytohospital.component.scss']
})
export class NoncompliancesnaquerytohospitalComponent implements OnInit {
  schemeidvalue: any
  schemeName: any
  currentPage: any;
  pageElement: any;
  ...
  getNONcompliancesna() {
    ...
    this.LoginServ.getnoncompliancesnaquery(this.hospitalCode, this.fromDate, this.toDate, this.urn, this.caseNo, this.schemeId, this.schemeCategoryId)
      .subscribe({
        next: (response: any) => {
          if (response && response.status === 200 && response.data) {
            this.getnoncompliancesnaquery = response.data;
            this.record = this.getnoncompliancesnaquery.length;
            this.showPegi = this.record > 0;
          } else {
            this.getnoncompliancesnaquery = [];
  ...
  heading = [['Sl#','URN','Case Number', 'Patient Name', 'Date of Admission', 'SNA Query Date','Remark','Description', 'Amount']];
  downloadReport(type: any) {
    this.report = [];
    if (type == 'excel') {
      let claim: any;
      for (var i = 0; i < this.getnoncompliancesnaquery.length; i++) {
        claim = this.getnoncompliancesnaquery[i];
        this.sno = [];
        this.sno.Slno = i + 1;
        ...
        this.sno.SNAQueryDate = claim.cpdactiondate;
      ...
      if (this.getnoncompliancesnaquery.length == 0) {
        this.swal('', 'No Data Found', 'info');
        return;
      }
      let SlNo = 1;
      this.getnoncompliancesnaquery.forEach(element => {
        let rowData = [];
        rowData.push(SlNo++);
