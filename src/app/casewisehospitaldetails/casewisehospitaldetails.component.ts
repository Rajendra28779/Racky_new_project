...
import { ClaimRaiseServiceService } from '../application/Services/claim-raise-service.service';
...
@Component({
  selector: 'app-casewisehospitaldetails',
  templateUrl: './casewisehospitaldetails.component.html',
  styleUrls: ['./casewisehospitaldetails.component.scss']
})
export class CasewisehospitaldetailsComponent implements OnInit {
  userdetailsByid: any = [];
  ...
  constructor(public headerService: HeaderService, private logser: ClaimRaiseServiceService, public router: Router,
    public snoService: SnoCLaimDetailsService, private jwtService: JwtService, private cpdService: CreatecpdserviceService) {
  }
  ...
  onSearchFilterClaimDetails() {
    let check = this.getid;
    console.log(check + " check");
    this.logser.gethospitalpatienttreatmentdetails(check).subscribe({
      next: (response: any) => {
        if (response && response.status === 200 && response.data && response.data.data) {
          ...
          if (this.hsptlpatientdtls.length > 0) {
            ...
            this.getPackageDetailsInfoList(this.hsptlpatientdtls[0].txnpackagedetailid);
          } else {
  ...
  ongetpreAuthdata() {
    ...
    this.cpdService.getPreAuthHistory(URNnumber, Authroziedcode, Hospitalcode).subscribe({
      next: (data: any) => {
        this.preAuth = data;
      ...
      error: (err: any) => {
        ...
        this.errorMessage = 'An error occurred while fetching the pre-authorization data. Please try again later.';
  ...
  getPackageDetails(packageCode, subPackageCode, procedureCode) {
    this.snoService.getPackageDetails(packageCode, subPackageCode, procedureCode, this.Hospital).subscribe(
      (data: any) => {
        let resData = data;
